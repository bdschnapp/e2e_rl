...
as the RL agent, a bounded steering rate, so the comparison between
...
the articulated vehicle. Driving forward, the tractor leads and the trailer follows
passively, so it suffices to regulate the tractor errors $(e_y, e_\psi)$ towards zero.
Driving in reverse, the trailer leads and the coupling inverts. The hitch angle
$\gamma$ becomes open-loop unstable (small articulation errors grow and the vehicle
...
reformulation (following the Autoware trailer LTV-MPC): the QP plans the trailer as
a stable bicycle driven by a virtual trailer steer $\delta_T$, and the hitch enters
...
analytic inner map realizes it on the physical tractor and supplies the anti-jackknife
counter-steer,
\begin{equation}
...
search is run separately for each controller and each direction, because the
...
in the same loop, on the same unfiltered evaluation pool, is what makes the reinforcement-learning
comparison in the results chapter fair. The measured numbers are
