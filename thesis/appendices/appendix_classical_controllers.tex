...
(\texttt{tractor\_trailer\_rl\_cupy}) so that they run inside the same vectorised
...
$(e_{y,t}, e_{\psi,t})$ and must additionally include an explicit hitch-stabilising action.
...
controllers are summarised in \cref{tab:app_cc_params}; here $L_1$ is the tractor
...
\end{equation}
Reverse, it regulates the trailer errors and adds an explicit hitch-stabilising term
...
vectorised across the parallel environments. Besides serving as a benchmark, the tuned
...
\end{equation}
The pair $(A_c, B_c)$ is discretised by zero-order hold at the step $\Delta t$ to give
...
algebraic Riccati equation. Its stabilising solution $P$ yields
\begin{equation}
...
every environment, so the controller stays fully vectorised. The command adds a
...
dynamics inside the optimisation causes the long shunt trailer to oscillate and
...
stabilising counter-steer.

\subsection{Condensed quadratic program}

The continuous models are discretised by zero-order hold and the state trajectory is
condensed onto the input sequence, so that the optimisation variable is the vector of
...
summarised in \cref{alg:app_cc_mpc}.

\begin{algorithm}[H]
\caption{Kinematic LTV MPC step (per environment)}
\label{alg:app_cc_mpc}
\begin{algorithmic}[1]
\State \textbf{Precompute (once):} discretise $(A_c, B_c, E_c)$ by ZOH; build the
...
\end{equation}
so that reliably completing the path is maximised first and, among gains that complete,
...
laboratory vehicle: on the shunt truck those gains destabilise the vehicle, and several
gains (notably the hitch-stabilising terms) change sign. This is a direct consequence of
