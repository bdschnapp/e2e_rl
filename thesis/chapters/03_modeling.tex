...
Reverse motion is characterised by a sign reversal in the kinematic equations, which transforms the stabilising effect of forward velocity into a destabilising force. Numerical experiments using NMPC have shown that successful reverse maneuvering requires optimisation of the steering input over a prediction horizon to prevent $\gamma$ from reaching its mechanical limits. The RL agent developed in this thesis must learn analogous foresight: rewarding actions that keep the trailer aligned with the target path while maintaining $\gamma$ within a safe operational envelope.
...
\begin{itemize}
    ...
    \item $\mathcal{T}$: transition dynamics simulated in Gazebo;
