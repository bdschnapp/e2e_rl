...
The training and evaluation environment is a custom 2D simulation built on the Farama Foundation Gymnasium library~\cite{towers2024gymnasium}. Gymnasium provides a standardised API for implementing environments that integrate directly with reinforcement learning algorithm libraries, including Stable Baselines3~\cite{raffin2021sb3}.
...
\begin{enumerate}
    \item \textbf{Vehicle Model}: A combined dynamic tractor (bicycle model with a Pacejka tire model~\cite{pacejka2012tyre} linearised to constant cornering stiffness, \cref{eq:tireforce}) and kinematic trailer with hitch constraints, using Tesla Model S parameters.
