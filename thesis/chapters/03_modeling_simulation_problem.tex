...
\Cref{fig:tractor_trailer_geometry} illustrates the tractor-trailer geometry, the tractor and trailer headings $\psi$ and $\psi_t$, and the articulation angle $\gamma = \psi - \psi_t$ used throughout this section. The hitch is drawn at the rear axle because the reference tractor places it just $0.25$\,m ahead of that axle, under a tenth of the $2.95$\,m tractor wheelbase, and the model retains the offset $L_h$ as a free parameter rather than assuming it away.
...
\begin{figure}[H]
  \centering
  \safegraphic[width=0.55\textwidth]{figures/pygame/sim_bev.png}{Top-down simulation view of the tractor and trailer inside the lane corridor.}
  \caption{The simulated scene from which the bird's-eye-view observation is cropped. The drivable lane corridor is white and everything outside it is blocked; the tractor is blue and the trailer orange. This top-down view, centered on the vehicle and reduced to a $32 \times 32$ crop, is the image the policy receives.}
  \label{fig:sim_bev}
\end{figure}
...
\subsection{Control Policy Interface}

A traditional or learning based policy interacts with the simulation through a deliberately narrow interface. In the reinforcement-learning sense its action is two-dimensional, a continuous steering rate $\dot{\delta}$ and a stop signal $\sigma$:
\begin{equation}
...
Episodes terminate on collision, jackknife ($|\gamma| > 90^\circ$), intentional stop, or successful task completion.
...
\input{chapters/sections/scalable_simulation}