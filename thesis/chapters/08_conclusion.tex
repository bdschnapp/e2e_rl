...
\begin{itemize}
    ...
    \item Hardware deployment is addressed in Chapter~\ref{ch:sim_to_real} via a ROS2/Autoware integration on the Agilex Hunter SE platform, but full characterisation of sim-to-real performance across the complete range of training tasks remains as future work.
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{End-to-end neural policy deployment}: The current hardware deployment uses LQR and MPC controllers with gains informed by simulation; future work should deploy the trained TD3 neural network directly on the Hunter SE, requiring latency compensation, domain randomisation during training, and a supervised safety fallback layer.
